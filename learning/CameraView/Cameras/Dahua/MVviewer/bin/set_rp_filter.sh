...
DisplayHelp()
{
    ...
    echo "    Allows the MV SDK to find GigE Vision devices that"
    ...
    echo ""
    echo "COPYRIGHT"
    echo "    Dahua Technologies Inc. Copyright (c) 2002-2016"
    echo ""
}
...
# If net.ipv4.conf.all.rp_filter present in /etc/sysctl.conf, change value to "$MODE"
#	else, add "net.ipv4.conf.all.rp_filter = $MODE" to /etc/sysctl.conf
if grep --silent ^net.ipv4.conf.all.rp_filter /etc/sysctl.conf; then
    ...
    sed -i '/net.ipv4.conf.default.rp_filter/a\net.ipv4.conf.all.rp_filter = '$MODE'' /etc/sysctl.conf
    echo "" >> /etc/sysctl.conf
fi
