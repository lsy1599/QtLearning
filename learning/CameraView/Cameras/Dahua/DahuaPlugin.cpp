...
bool DahuaPlugin::Discover()
{
    ...
    if(Refresh())
    {
        for(size_t index = 0; index < m_vCameraPtrList.size(); index++)
        {
            ...
            CameraDevice*  cameraDev =new  CameraDahua(m_systemObj, cameraSptr);
            cameraDev->CameraIndex  = static_cast<int>(index);
            cameraDev->VendorName   = cameraSptr->getVendorName();
            cameraDev->ModelName    = cameraSptr->getModelName();
            cameraDev->SerialNumber = cameraSptr->getSerialNumber();
            ...
            case ICamera::typeGige:
            {
                ...
                {
                    cameraDev->Ipaddress = gigeCameraPtr->getIpAddress().c_str();
                }
                break;
            }
...
bool DahuaPlugin::Refresh()
{
    bool isDiscoverySuccess = m_systemObj.discovery(m_vCameraPtrList);
...
int DahuaPlugin::CameraNumber()
{
    return _Cameras.size();
}

CameraDevice *DahuaPlugin::operator[](int index)
{
...
DahuaPlugin::CameraDahua::CameraDahua(CSystem &systemObj, ICameraPtr &cameraSptr)
    :_cameraSptr(cameraSptr) , _systemObj(systemObj)
{

}

bool DahuaPlugin::CameraDahua::Open()
{
    if(!_cameraSptr->connect())
    {
        qDebug("connect cameral failed.");
        return false;
    }

    /* 设置相机为连续拉流模式 */
    setGrabMode(_cameraSptr, true);

    setExposureTime(_cameraSptr, 10000, false);
...
bool DahuaPlugin::CameraDahua::Start()
{
    /* 创建流对象 */
    _streamPtr = _systemObj.createStreamSource(_cameraSptr);
    if (nullptr == _streamPtr)
    {
    ...
    /* 开始取图 */
    bool isStartGrabbingSuccess = _streamPtr->startGrabbing();
...
bool DahuaPlugin::CameraDahua::Stop()
{
    /* 停止拉流线程 */
    _streamPtr->stopGrabbing();

    /* 修改相机曝光时间 */
    modifyCamralExposureTime(_systemObj, _cameraSptr);

    return true;
}

bool DahuaPlugin::CameraDahua::Close()
{
    /* 断开相机 */
    if (!_cameraSptr->disConnect())
    {
        qDebug("disConnect camera failed");
        return false;
    }
    return true;
}
...
bool DahuaPlugin::CameraDahua::GetOneImage(QImage &img, int timeout)
{
    if(_streamPtr)
    {
        ...
        if(isSuccess)
        {
            if(frame.valid())
            {
                QImage srcImage(static_cast<const uchar*>(frame.getImage()), static_cast<int>(frame.getImageWidth()),
                                static_cast<int>(frame.getImageHeight()), QImage::Format_Indexed8);
                img = srcImage.copy();
                return true;
            }else
