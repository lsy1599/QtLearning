...
void AlgorithmEdit::InitToolBar()
{
    ...
    act = ToolBar->addAction(QIcon(":/Algorithm/ico/preview.ico"), QString("view"));
    act->setToolTip(QStringLiteral("训练预览"));
    act->setCheckable(true);
    shapeGroup->addAction(act);
...
void AlgorithmEdit::setShape(QAction *act)
{
    if(act)
    {
        if(act->isChecked())
        {
            ...
            } else if (act->text() == "view") {
                Scene->setShape(GraphicsScene::VIEW);
                act->setChecked(false);
            }
        }
    }
